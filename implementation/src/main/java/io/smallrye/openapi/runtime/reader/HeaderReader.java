...
public class HeaderReader {
    ...
    public static Map<String, Header> readHeaders(final AnnotationScannerContext context,
        ...
        for (AnnotationInstance nested : nestedArray) {
            String name = JandexUtil.stringValue(nested, MPOpenApiConstants.HEADER.PROP_NAME);
    ...
    private static Header readHeader(final AnnotationScannerContext context, final AnnotationInstance annotationInstance) {
        ...
        header.setDescription(JandexUtil.stringValue(annotationInstance, MPOpenApiConstants.HEADER.PROP_DESCRIPTION));
        header.setSchema(
                SchemaFactory.readSchema(context.getIndex(), annotationInstance.value(MPOpenApiConstants.HEADER.PROP_SCHEMA)));
        header.setRequired(JandexUtil.booleanValue(annotationInstance, MPOpenApiConstants.HEADER.PROP_REQUIRED));
        header.setDeprecated(JandexUtil.booleanValue(annotationInstance, MPOpenApiConstants.HEADER.PROP_DEPRECATED));
        header.setAllowEmptyValue(
                JandexUtil.booleanValue(annotationInstance, MPOpenApiConstants.HEADER.PROP_ALLOW_EMPTY_VALUE));
        // ? Style
        // ? Explode
        // ? Example
        // ? Examples
    ...
    private static Header readHeader(final JsonNode node) {
        ...
        Header header = new HeaderImpl();
        header.setRef(JsonUtil.stringProperty(node, MPOpenApiConstants.HEADER.PROP_REF_VAR));
        header.setDescription(JsonUtil.stringProperty(node, MPOpenApiConstants.HEADER.PROP_DESCRIPTION));
        header.setSchema(SchemaReader.readSchema(node.get(MPOpenApiConstants.HEADER.PROP_SCHEMA)));
        header.setRequired(JsonUtil.booleanProperty(node, MPOpenApiConstants.HEADER.PROP_REQUIRED));
        header.setDeprecated(JsonUtil.booleanProperty(node, MPOpenApiConstants.HEADER.PROP_DEPRECATED));
        header.setAllowEmptyValue(JsonUtil.booleanProperty(node, MPOpenApiConstants.HEADER.PROP_ALLOW_EMPTY_VALUE));
        header.setStyle(readHeaderStyle(node.get(MPOpenApiConstants.HEADER.PROP_STYLE)));
        header.setExplode(JsonUtil.booleanProperty(node, MPOpenApiConstants.HEADER.PROP_EXPLODE));
        header.setExample(readObject(node.get(MPOpenApiConstants.HEADER.PROP_EXAMPLE)));
        header.setExamples(ExampleReader.readExamples(node.get(MPOpenApiConstants.HEADER.PROP_EXAMPLES)));
        header.setContent(ContentReader.readContent(node.get(MPOpenApiConstants.HEADER.PROP_CONTENT)));
