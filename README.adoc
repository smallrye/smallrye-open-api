...
The `x-smallrye-profile-{profileName}` extension provides a way to include or exclude an application's resources in the OpenAPI model. When placed on a REST endpoint method, the extension `name` specifies a profile name that is used together with one of the <<user-content-scan-profiles,scan profile>> configuration properties.
