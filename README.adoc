...
----
Specify a strategy to be used globally for deriving operation IDs when not specified via an annotation or static OpenAPI input. Set to one of the following:
** `METHOD`
** `CLASS_METHOD`
** `PACKAGE_CLASS_METHOD`
** A fully-qualified class name of an implementation of `io.smallrye.openapi.api.OpenApiConfig.OperationIdStrategy` (as of 4.1.2, https://javadoc.io/doc/io.smallrye/smallrye-open-api-core/latest/io/smallrye/openapi/api/OpenApiConfig.OperationIdStrategy.html[see JavaDoc])
